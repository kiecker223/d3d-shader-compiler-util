...
SHADER CompileVertexShader(const std::string& InByteCode)
{
...
SHADER CompilePixelShader(const std::string& InByteCode)
{
	if (CompilationMode == DXIL)
	{
...
SHADER CompileHullShader(const std::string& InByteCode)
{
	if (CompilationMode == DXIL)
	{
...
SHADER CompileDomainShader(const std::string& InByteCode)
{
	if (CompilationMode == DXIL)
	{
...
SHADER CompileGeometryShader(const std::string& InByteCode)
{
	if (CompilationMode == DXIL)
	{
...
SHADER CompileComputeShader(const std::string& InByteCode)
{
	if (CompilationMode == DXIL)
	{
...
SHADER CompileRaytracingShader(const std::string& InByteCode)
{
	if (CompilationMode == DXIL)
	{
...
static bool HasDomainShader(const FULL_PIPELINE_DESCRIPTOR& Desc)
{
...
static FULL_PIPELINE_DESCRIPTOR CreateDefaultDescriptor()
{
	...
	ZeroMemory(&Result, sizeof(FULL_PIPELINE_DESCRIPTOR));
...
FULL_PIPELINE_DESCRIPTOR ParseGraphicsShader(const std::string& InFile, const std::string& FileName)
{
	...
	{
		uint8_t& OutNumConstantBuffers		= PipelineDesc.Counts.NumConstantBuffers;
		uint8_t& OutNumSamplers				= PipelineDesc.Counts.NumSamplers;
		uint8_t& OutNumShaderResourceViews	= PipelineDesc.Counts.NumShaderResourceViews;
		uint8_t& OutNumUnorderedAccessViews	= PipelineDesc.Counts.NumUnorderedAccessViews;
...

	{
		...
		if (bHasDomainShader)
		{
			PipelineDesc.DS = CompileDomainShader(FullShader);
...
COMPUTE_PIPELINE_DESC ParseComputeShader(const std::string& InFile, const std::string& FileName)
{
	...
	{
		uint8_t& OutNumConstantBuffers = Result.Counts.NumConstantBuffers;
		uint8_t& OutNumSamplers = Result.Counts.NumSamplers;
		uint8_t& OutNumShaderResourceViews = Result.Counts.NumShaderResourceViews;
		uint8_t& OutNumUnorderedAccessViews = Result.Counts.NumUnorderedAccessViews;
...
COMPUTE_PIPELINE_DESC ParseRaytracingShader(const std::string& InFile, const std::string& FileName)
{
	COMPUTE_PIPELINE_DESC Result = {};
	...
	Result.CS = CompileRaytracingShader(InFile);
...
COMPUTE_PIPELINE_DESC LoadRaytracingPipeline(const std::string& ShaderFile)
{
	std::string File = ReadEntireFile(ShaderFile);
	COMPUTE_PIPELINE_DESC PipelineDesc = ParseRaytracingShader(File, ShaderFile);
	return PipelineDesc;
}
...
void WriteBufferToFile(const std::string& FileName, const byte* Data, uint32_t DataLen)
{
	EnsureFileDirectoryExists(FileName);
	HANDLE File = CreateFileA(FileName.c_str(), FILE_WRITE_ACCESS, FILE_SHARE_WRITE, nullptr, CREATE_ALWAYS, 0, nullptr);
...
{
	COMPUTE_PIPELINE_DESC Pipeline = LoadRaytracingPipeline(SearchedFolder + '/' + SourceFile);
	json PipelineDescriptor;
	PipelineDescriptor["NumConstantBuffers"] = Pipeline.Counts.NumConstantBuffers;
	PipelineDescriptor["NumSamplers"] = Pipeline.Counts.NumSamplers;
	PipelineDescriptor["NumShaderResourceViews"] = Pipeline.Counts.NumShaderResourceViews;
